...
SELECT  V.Name, V.PhoneNumber, V.Address, V.AnimalId,V.DepartmentId FROM Volunteers AS V
ORDER BY V.Name, V.AnimalId, V.DepartmentId ASC

SELECT A.Name, A.AnimalTypeId, A.BirthDate FROM Animals AS A 
ORDER BY A.Name ASC

SELECT TOP 5 O.Name, COUNT(A.Id) AS AnimalsCount  FROM Owners AS O
Join Animals AS A ON A.OwnerId = O.Id
GROUP BY O.Name 
ORDER BY AnimalsCount DESC, O.Name 

SELECT CONCAT(O.Name,'-', A.Name) AS OwnersAnimals, O.PhoneNumber, C.Id  FROM Owners AS O
Join Animals AS A ON A.OwnerId = O.Id
Join Cages AS C ON C.AnimalTypeId = A.AnimalTypeId
